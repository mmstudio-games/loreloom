...
- [x] 实现 fixed/preset/UGC 玩家创建 Content Schema、七类字段、固定 binding/effect、类型化 Draft、
  CharacterSpawnSpec/Parameter 初值编译、PlayerCreated provenance 与创建前全量校验；
- [x] 实现 preset/UGC TUI 选择与表单交互、预览、双向键盘导航、逐字段即时验证与错误定位，并覆盖
  无 Provider 创建、失败不留存档、round-trip 和确定性渲染测试；
